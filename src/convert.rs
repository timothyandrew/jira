    ...
    match &node.data.borrow().value {
        NodeValue::Document => { 
            Node::Root {
                ...
                content: node.children().map(|c| convert_node_markdown_to_adf(c, None)).collect() 
            }
        }
        ...
        NodeValue::DescriptionList => { panic!("Definition lists are unsupported!"); }
        NodeValue::DescriptionItem(_) => { panic!("Definition lists are unsupported!"); }
        NodeValue::DescriptionTerm => { panic!("Definition lists are unsupported!"); }
        NodeValue::DescriptionDetails => { panic!("Definition lists are unsupported!"); }
        ...
        NodeValue::HtmlBlock(_) => { panic!("HTML blocks are unsupported!"); }
        NodeValue::Paragraph => {
            Node::BlockNode {
        ...
        NodeValue::FootnoteDefinition(_) => { panic!("Footnotes aren't supported!"); }
        NodeValue::Table(_) => {
            Node::BlockNode {
        ...
        NodeValue::Text(text) => {
            Node::InlineNode {
                ...
                text: Some(std::str::from_utf8(&text[..]).expect("Invalid UTF-8!").to_owned()),
                marks
            }
        }
        NodeValue::TaskItem(_) => { panic!("Task lists not supported!"); }
        NodeValue::SoftBreak => {
            Node::InlineNode {
                nodetype: InlineNodeType::Text,
                text: Some(" ".to_owned()),
        ...
        NodeValue::HtmlInline(_) => { panic!("Raw HTML not supported!"); }
        NodeValue::Emph => {
            let marks = append(marks, Mark::Em);
            convert_node_markdown_to_adf(node.first_child().expect("Nothing to emphasize!"), Some(marks))
        }
        NodeValue::Strong => {
            let marks = append(marks, Mark::Strong);
            convert_node_markdown_to_adf(node.first_child().expect("Nothing to embolden!"), Some(marks))
        }
        NodeValue::Strikethrough => {
            let marks = append(marks, Mark::Strike);
            convert_node_markdown_to_adf(node.first_child().expect("Nothing to strikethrough!"), Some(marks))
        }
        NodeValue::Superscript => {
            let mark = Mark::Subsup { subsuptype: SubsupType::Sup };
            let marks = append(marks, mark);
            convert_node_markdown_to_adf(node.first_child().expect("Nothing to superscript!"), Some(marks))
        }
        NodeValue::Link(link) => {
            let mark = Mark::Link {
                href: std::str::from_utf8(&link.url[..]).expect("Invalid UTF-8!").to_owned(),
                title: std::str::from_utf8(&link.title[..]).expect("Invalid UTF-8!").to_owned(),
            };

            let marks = append(marks, mark);
            convert_node_markdown_to_adf(node.first_child().expect("Nothing to superscript!"), Some(marks))
        }
        NodeValue::Image(_) => { panic!("Images aren't supported, sorry!"); }
        NodeValue::FootnoteReference(_) => { panic!("Footnotes aren't supported!"); }
    }
}
